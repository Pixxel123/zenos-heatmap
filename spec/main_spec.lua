...
describe("plugin entry", function()
    ...
    it("picks the height from the range unless overridden", function()
        local ZenHeatmap = require("main")
        assert.equals("s", ZenHeatmap.sizeFor({ range = "year", size = "auto" }))
        assert.equals("m", ZenHeatmap.sizeFor({ range = "quarter", size = "auto" }))
        assert.equals("m", ZenHeatmap.sizeFor({ range = "month", size = "auto" }))
